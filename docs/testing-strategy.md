...
- **Scenario matrix:** A ingest→status (isMeta turn dropped) · B persistence across a process restart · C export→import round-trip (replace/merge) · D MCP tool contract · E forget two-phase (single-use handle/replay) · F consolidate (dry-run/write/idempotent/`--force`) · G install-hooks + `abs hook` (snake_case payload, always exit 0) · H optimize preview→apply (CLAUDE.md + backup) + protected-memory guard · K cross-project isolation (#47) · **M auto-distill cadence `abs maintain --auto` (#138): consolidate → auto-memory written + `MEMORY.md` pointer, `CLAUDE.md` untouched, idempotent second run, benign no-op with no LLM)** · I UI graph + DOM-driven "excluir busca" delete · J UI write-path gates (CSRF/Origin 403, method 405, bad handle 409).
- **Cadence / ingest / MCP-contract / payload changes must run the e2e Vitest lane by hand before the PR leaves draft** (`npm run build && npm run test:e2e:vitest`, or the full `npm run test:e2e`). CI runs `test:e2e:vitest` on every PR (#113), but build it locally first — `npm run check` does **not** rebuild `dist/`, so the e2e suite would otherwise drive a stale binary. The cadence scenario M scopes its optimize pass to `process.cwd()`, so its project root is the **realpath** of the temp dir (the macOS `/var → /private/var` symlink would otherwise make `projectSlug(process.cwd())` diverge from the ingested slug).
