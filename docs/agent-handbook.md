...
- Set/confirm/skip the CURRENT session's project (writes the decision binding of ADR 0010, applied at next ingest): `abs project [--set NAME | --cwd | --skip] [--session ID] [--yes] [--json]`. Session id resolves from `CLAUDE_CODE_SESSION_ID` (NOT latest-transcript-by-mtime — subagents pollute the dir), `--session` overrides. `--skip` hard-deletes any already-stored observations for the session → requires `--yes` (ADR 0008). No-arg prints status + suggestions.
- MCP tools exposed to agents: `recall`, `remember`, `memory_status`, `optimize`/`apply` (gated edits), `forget_preview`/`forget` (two-phase selective delete: preview mints a single-use handle, `forget` consumes it — IRREVERSIBLE; ADR 0008), and `set_session_project` (#52: record this session's project — `action=set|skip`; pass `session` from the SessionStart picker, `CLAUDE_CODE_SESSION_ID` fallback; `skip`'s hard-delete of stored observations needs `confirmDelete=true`, the MCP analog of `abs project --skip --yes`; ADR 0008/0010). The UI write path (delete) is gated by the localhost CSRF/Origin controls in ADR 0007.
- SessionStart injects, besides the baseline, a **project picker** (#52, ADR 0010): an imperative instruction telling Claude to ask the user which project this session belongs to (existing / new / auto-slug / skip) and record it via `set_session_project`. $0/offline (`ensure:false`), non-fatal, suppressed once a binding exists (idempotent on resume) or when the payload has no session id. Soft — the hard guarantee is the CLI `abs project` (#51).
- **Recall is project-scoped by default** (#47): per-prompt injection, the MCP `recall` tool, and `recall`/`recallFts` filter to the current session's project so memory from other projects never leaks in. `ABS_RECALL_SCOPE=global` opts back into store-wide recall. The scope label is resolved (`src/recall/scope.ts` `resolveRecallProject`) from the **stored** label — `set` binding → stored `sessions.project` row → `basename(dirname(transcript_path))` (byte-identical to what ingest stores) → `projectSlug(cwd)` last resort — never a freshly recomputed slug, so worktree/relabeled sessions don't silently starve. The injected fence header names the active project. Store legs (`searchFts`/`knn`) apply the filter in SQL (JOIN `sessions` / `rowid IN`), never a TS post-filter.
...
- **Ingest is decision-aware** (ADR 0010). A session's project is auto-derived from the transcript dir slug unless an intentional binding exists in `kv_meta` (`session-project:<sessionId>`): `set X` overrides the project (UPDATE-safe), `skip` excludes the session entirely (no row, cursor still advances; reconciles an already-ingested session). No binding → byte-for-byte the auto behavior. The binding is at-least-once (applied whenever ingest next runs — `SessionEnd` is not guaranteed on abrupt kill, validated by spike #49). Writers: CLI `abs project` (#51) / MCP tool (#52); consumer: project-scoped recall (#47).
