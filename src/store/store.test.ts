...
describe('MemoryStore', () => {
  ...
  describe('deleteSessionsByProject', () => {
    /** Seed one session + one indexed observation, return [sessionId, obsId]. */
    function seed(project: string | null, hot: number): [number, number] {
      const sessionId = store.createSession({
        externalId: `ext-${project ?? 'NULL'}-${hot}`,
        ...(project !== null ? { project } : {}),
      });
      const obsId = store.createObservation({ sessionId, kind: 'user', content: `c${hot}` });
      store.upsertVector(obsId, unitVector(hot));
      store.indexFts(obsId, `c${hot}`);
      return [sessionId, obsId];
    }

    it('deletes all sessions of a project and prunes vec0 + fts5 in lockstep', () => {
      seed('alpha', 0);
      seed('alpha', 1);
      seed('beta', 2);

      const res = store.deleteSessionsByProject('alpha');
      expect(res).toEqual({ sessions: 2, observations: 2 });

      const c = store.counts();
      expect(c).toMatchObject({ sessions: 1, observations: 1, vectors: 1, fts: 1 });
      // index rows match observation rows exactly — no orphans, nothing over-pruned.
      expect(c.vectors).toBe(c.observations);
      expect(c.fts).toBe(c.observations);
    });

    it('null selector matches NULL project only — not the literal string "null"', () => {
      seed(null, 0);
      seed('null', 1); // the literal three-letter string

      const res = store.deleteSessionsByProject(null);
      expect(res).toEqual({ sessions: 1, observations: 1 });
      // the literal-'null' session survives, fully indexed.
      expect(store.counts()).toMatchObject({ sessions: 1, observations: 1, vectors: 1, fts: 1 });
    });

    it('string "null" selector does NOT match a NULL-project session', () => {
      seed(null, 0);
      seed('null', 1);

      const res = store.deleteSessionsByProject('null');
      expect(res).toEqual({ sessions: 1, observations: 1 });
      // the NULL-project session survives.
      expect(store.counts()).toMatchObject({ sessions: 1, observations: 1, vectors: 1, fts: 1 });
    });

    it('returns zeros and mutates nothing when no session matches', () => {
      seed('alpha', 0);
      const res = store.deleteSessionsByProject('does-not-exist');
      expect(res).toEqual({ sessions: 0, observations: 0 });
      expect(store.counts()).toMatchObject({ sessions: 1, observations: 1, vectors: 1, fts: 1 });
    });

    it('removes every observation across multiple sessions of one project', () => {
      const [s1] = seed('proj', 0);
      // a second observation on the same first session
      const extra = store.createObservation({ sessionId: s1, kind: 'user', content: 'extra' });
      store.upsertVector(extra, unitVector(5));
      store.indexFts(extra, 'extra');
      seed('proj', 1);

      const res = store.deleteSessionsByProject('proj');
      expect(res).toEqual({ sessions: 2, observations: 3 });
      expect(store.counts()).toMatchObject({ sessions: 0, observations: 0, vectors: 0, fts: 0 });
    });
  });
